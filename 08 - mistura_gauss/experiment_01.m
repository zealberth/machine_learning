% Using Gaussian Mixture Models to classify
...
%% experiment setup
n_repetition = 30;
dataset_names = {'iris'};      % conjuntos de dados 'column2C', 'bcw' , 'iris', 'column3C', 'dermatology' 
...
for i=1:length(dataset_names)
    ...
    for j=1:n_repetition
        ...
        %% train
        k = 5;
        [mi, sigma, alfa, p_w] = mix_gauss_train(data.x_train, data.y_train, k);        
		%% test
        fprintf('Executando classificação...\n');
        y_hat = mix_gauss_predict(data.x_test, mi, sigma, alfa, k, p_w);        
		%% get metrics        
        % confusion matrix        
