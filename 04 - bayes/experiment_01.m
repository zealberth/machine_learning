% Cauculate accuracy using Bayes Classifier
...
%% experiment setup
n_repetition = 30;
dataset_names = {'column2C'};      % conjuntos de dados 'column2C', 'bcw' , 'iris', 'column3C', 'dermatology' 
...
for i=1:length(dataset_names)
    ...
    for j=1:n_repetition
        ...
        data = dataset(j);
        %% train
        [mi, sigma, p_w] = bayesmulti_train(data.x_train, data.y_train);        
		%% test
        fprintf('Executando classificação...\n');
        y_hat = bayesmulti_predict(data.x_test, mi, sigma, p_w);        
		%% get metrics        
        % confusion matrix        
